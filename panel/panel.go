...
import (
	...
	"github.com/HenZenKuriRIP/XrayR4u/api/v2board"
	"github.com/HenZenKuriRIP/XrayR4u/app/mydispatcher"
	_ "github.com/HenZenKuriRIP/XrayR4u/main/distro/all"
	"github.com/HenZenKuriRIP/XrayR4u/service"
	"github.com/HenZenKuriRIP/XrayR4u/service/controller"
...
func (p *Panel) Start() {
	...
	for _, s := range p.Service {
		err := s.Start()
		if err != nil {
			log.Printf("[Panel] Failed to start service: %s", err)
		}
	}
	p.Running = true
	return
...
func (p *Panel) Close() {
	...
	for _, s := range p.Service {
		if err := s.Close(); err != nil {
			// Never panic on close: hot-reload must be able to tear down a
			// partially failed multi-node setup and start again.
