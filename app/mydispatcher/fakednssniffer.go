	...
	return protocolSnifferWithMetadata{protocolSniffer: func(ctx context.Context, bytes []byte) (SniffResult, error) {
		...
		if Target.Network == net.Network_TCP || Target.Network == net.Network_UDP {
			...
			if domainFromFakeDNS != "" {
				errors.LogInfo(ctx, "fake dns got domain: ", domainFromFakeDNS, " for ip: ", Target.Address.String())
	...
	return protocolSnifferWithMetadata{
		protocolSniffer: func(ctx context.Context, bytes []byte) (SniffResult, error) {
			...
			if ipAddressInRangeValue.addressInRange != nil {
				...
				errors.LogDebug(context.Background(), "ip address not in fake dns range, return as is")
				return nil, common.ErrNoClue
			}
			errors.LogWarning(context.Background(), "fake dns sniffer did not set address in range option, assume false.")
			return nil, common.ErrNoClue
		},
